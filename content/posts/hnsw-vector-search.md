...
date: 2026-03-05
description: "Most people using vector databases have no idea what HNSW is doing under the hood. Here's the actual algorithm, the tradeoffs, and when you should care."
...
Reading the [original HNSW paper](https://arxiv.org/abs/1603.09320) is actually worth your time. You won't implement it yourself. The algorithm's structure explains every tradeoff you'll encounter in production, though, and that's the useful part.

Here's what's happening.

## The problem HNSW is solving

Finding the nearest neighbor to a query vector in a dataset of a million 1536-dimensional floats is not fast if you do it naively. The brute-force approach -- compute cosine similarity to every vector, return the top k -- scales linearly with dataset size. At 10 million vectors, that's slow. At 100 million, it's unusable for anything interactive.

The standard computer science move is to trade exactness for speed. You find the *approximate* nearest neighbors. HNSW (Hierarchical Navigable Small World graphs) is the dominant approach right now across Qdrant, Weaviate, Chroma, FAISS, and most other vector databases. The paper, by Malkov and Yashunin, was submitted to arXiv in 2016 and published in IEEE TPAMI in 2018. It won out over the competition because it achieves very high recall at very low latency, and it does it in a structure that's easy to reason about once you understand the layers.
...
For 1 million OpenAI text-embedding-3-small vectors (1536 dimensions, float32): that's ~6.1 GB of raw vector data. With M=32, add another ~128 MB for connection overhead. Fine. But if you're indexing 100 million vectors, the memory requirements become the constraint, not the query performance.
...
Standard HNSW has no native support for filtered search. If you need "find me the 10 most similar vectors where metadata.user_id = 42", you have two options: pre-filter (reduce the dataset before searching) or post-filter (search the full index, then filter results).
...
[ScaNN](https://arxiv.org/abs/1908.10396) (ICML 2020) doesn't use HNSW at all. It uses anisotropic vector quantization: it quantizes vectors in a way that penalizes errors in the direction parallel to the query more than errors in orthogonal directions. The idea is that parallel errors affect the dot product more than orthogonal ones, so your quantization should be query-aware.

Google claimed 2x performance over other libraries on ann-benchmarks.com at the time. The [ann-benchmarks site](http://ann-benchmarks.com/) is the canonical place to see how different algorithms compare across datasets -- it runs automated benchmarks on glove-100, sift-128, nytimes-256 and others, and publishes recall vs. queries-per-second plots for 38 algorithms. If you're evaluating which index to use for a specific dataset and dimensionality, that's the place to start.

## The parameters you'll actually tune

If you're using a vector database today, here's what the research says matters:

**M:** Set higher for better recall, lower for lower memory and faster builds. Weaviate and Qdrant both default to M=16 or M=32. For most use cases, M=16 is fine. M=32 if recall is critical and you have the memory.

**ef_construction:** This is a build-time investment. Higher ef_construction doesn't affect memory or query time -- only graph quality. If you're indexing infrequently and querying constantly, this is worth setting high (200+). Qdrant's accurate config uses 100; doubling that won't break anything but will slow your index build.

**efSearch (or ef at query time):** This is the dial you turn when you need more recall without rebuilding the index. Start low, measure recall against a held-out dataset, increase until you hit your recall target. The latency cost is linear with ef. If recall is already fine, leave ef alone.

**Quantization:** If memory is tight, INT8 scalar quantization (4x compression, modest accuracy loss) or binary quantization (32x compression, ~74% raw recall) are worth evaluating. With oversampling on binary quantization, you can recover most of the recall loss. Weaviate's [binary quantization docs](https://weaviate.io/blog/binary-quantization) have the actual numbers.

---

The core thing to take away: HNSW is an approximate algorithm, every parameter is a tradeoff surface, and the tradeoffs are predictable once you understand the structure. M controls memory and density. ef controls query-time recall and speed. ef_construction controls build-time graph quality. The rest is implementation details.

If you're hitting performance issues with a vector DB and don't know where to start, the parameters are almost always the answer before architecture changes are.
