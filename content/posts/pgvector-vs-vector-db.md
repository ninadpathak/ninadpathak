...
**What's your p99 latency under production load?** Benchmarks in isolation don't capture shared buffer pool contention, Postgres vacuum overhead, or checkpoint interference. A database handling a mix of OLTP and vector workloads forces the vector queries to compete for resources with everything else.
...
[Supabase Vector](https://supabase.com/vector) and [Neon](https://neon.tech/) have made pgvector operationally simpler by hosting it as a managed service with sensible defaults. Already on managed Postgres and under ~5 million vectors, starting with pgvector and migrating when needed is a reasonable strategy.

## The case for dedicated databases

At scale, ten million or more vectors with high query volume and strict p95 latency requirements, dedicated databases have real advantages that aren't just marketing.
...
Consider a dedicated database when:

- You're approaching 10 million or more vectors
