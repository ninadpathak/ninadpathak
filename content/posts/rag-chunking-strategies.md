...
The retrieval quality ceiling for any RAG system gets set by chunking quality. A great embedding model retrieving bad chunks will consistently underperform a mediocre model retrieving good ones. The LlamaIndex benchmarks from 2024 showed retrieval precision varying by over 30 percentage points across chunking strategies on the same corpus. That's not a marginal difference. It's the difference between a system that works and one that doesn't.
...
The problem is mechanical. Text has semantic units: a sentence, a paragraph, a section. Fixed-size splitting ignores all of them. A chunk boundary can land in the middle of a code example, between a function definition and its parameters, or cut off an explanation right when it becomes useful.
...
Greg Kamradt's chunking experiments formalized this and showed it produces chunks with more coherent content than fixed-size or recursive approaches on narrative and technical documents. Each chunk is semantically unified because the splits happen where topics actually change.
