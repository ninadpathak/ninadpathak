...
The [multi-agent vs single-agent tradeoffs](/blog/multi-agent-vs-single-agent-tradeoffs/) article covers when multi-agent pipelines make sense in general. The short version: use them when the task decomposes cleanly and each step requires a different model or tool set.
...
The [memory hierarchy in AI systems](/blog/memory-hierarchy-in-ai-systems/) article covers how to organize memory layers in agentic systems. The architecture you choose for multi-agent memory should fit within that broader hierarchy. Shared memory in a multi-agent context is usually a working memory layer that all agents can read and write.
