...
A user with a billing issue would land in session three and explain their problem again from scratch, because the agent had no memory of the previous two sessions. It was not a context window problem. The context window was large enough. The problem was architectural: the agent had no persistent memory layer, and I had not thought about what that would cost until I watched a frustrated user type "as I mentioned last week" and watched the agent respond as if hearing it for the first time, asking for the account email it had verified twice already.
...
Over a two-week period on a live deployment, I measured that 34% of returning customers had to repeat information they had already provided in a previous session. Those repeat explanations added an average of 4 minutes to handle time per session. Across 200 returning customers per day, that is 800 minutes of wasted time per day, roughly 13 hours of agent capacity consumed by a memory problem.

A larger context window does not fix this, because the context window starts empty every session no matter how big it is. What fixes it is a persistent memory layer that survives sessions and gives the agent access to what happened before. The [state of AI agent memory in 2026](/blog/state-of-ai-agent-memory-2026/) documents how the tooling for this has matured, but the architectural decision of what to persist and what to discard still belongs to you as the system designer.
...
**Working memory** lives in the context window. It holds the current conversation, the immediate facts needed to generate responses, and the session-specific context. Being ephemeral by design, it disappears the moment the session ends.

**Episodic memory** stores records of past support interactions. Each episode is a structured record: customer ID, timestamp, problem category, what was diagnosed, what resolution was attempted, and the outcome. A customer returning after two weeks triggers a retrieval of relevant episodes before the conversation even starts, so the agent opens already knowing that last time the fix was to reset their API key.

**Semantic memory** stores customer profile data that changes infrequently: plan type, billing information, known preferences, account configuration. Think of it as the stable ground truth about the customer that does not vary session to session, the equivalent of the header on a file folder rather than the loose pages inside it. If you want a deeper map of how [episodic, semantic, and working memory](/blog/episodic-vs-semantic-vs-working-memory-agents/) differ in agents, I covered the distinctions in detail elsewhere.
...
My first implementation retrieved all episodes for a customer at session start and injected them into context. For customers with 5-10 episodes, that worked fine. It catastrophically failed for customers with 200 past interactions, where the retrieval injection alone consumed 40% of a 128k context window before the customer had typed a word.

Asymmetric retrieval was the fix, which I wrote about in detail in my post on [why agent memory retrieval is asymmetric](/blog/asymmetric-retrieval-agent-memory/). The principle applies here: you do not retrieve everything, you retrieve what is relevant to the current query.
...
My fix was to add explicit attribution metadata to each memory retrieval. The prompt now includes a step where the agent must state which specific facts from retrieved memory are relevant to the current query and which are not, the way a good doctor reads a chart and explicitly notes that the old knee injury has nothing to do with today's cough. Forcing that attribution reduced attribution errors by roughly 60% in my testing.
...
A returning customer would interact with the agent and not realize the agent already knew their history. They would provide background information that contradicted what was in memory, because they had forgotten what they had reported before, even though the memory itself was accurate. That mismatch created a new class of errors: the agent would cite memory the customer no longer agreed with, and the customer would feel the agent was gaslighting them. One person insisted they had never enabled two-factor auth, then went quiet when the agent referenced the exact ticket where they had asked us to turn it on.
...
Do not skip the attribution step. Retrieval was the part I got working in a day. Getting the agent to correctly use what it retrieves, instead of dragging an irrelevant 8-month-old ticket into today's conversation, is where the stubborn problems live. Build the attribution check into the prompt from the start, not as a later addition.
...
The full memory architecture I describe here is specific to customer support, but the underlying principles apply broadly. Any agent that needs to maintain a relationship with a person, rather than just answer a single question, needs a persistent memory layer instead of a fresh context window each time. Confusing those two things is where most agent deployments break.
