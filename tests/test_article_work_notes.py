    ...
    def test_published_notes_are_present_unique_and_short(self):
        ...
        for path in Path("content/posts").glob("*.md"):
            ...
            with self.subTest(path=path):
                ...
                self.assertEqual(text.count("Ninad Pathak"), 1)
                self.assertRegex(text, r"(?i)technical(?: \w+){0,2} (?:content|writer|documentation|comparisons|guides)|developer (?:content|tutorials)")
                self.assertNotRegex(text, r"(?i)\b(?:I|me|my|mine|we|our|ours|us)\b|let['’]s")
                notes.append(note)
