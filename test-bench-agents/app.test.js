const { incrementCounter, resetCounter } = require('./app');
...
// Skip the random delay in tests
...
describe('incrementCounter', () => {
    ...
    test('no lost increments under concurrent calls', async () => {
        ...
        // All 10 values should be unique (no two calls got the same counter value)
        expect(new Set(results).size).toBe(10);
        // Final counter must be 10
    ...
    test('resetCounter resets global state', async () => {
        await incrementCounter();
        resetCounter();
        const result = await incrementCounter();
        expect(result).toBe(1);
