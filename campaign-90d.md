...
**Final correction after building the missing instrument:** the other page was not unmeasured. `tools/gsc_page_position.py` makes stable pages visible and shows both candidates as `withheld`, not `never-impressed`, over 28 days. On identical page-dimension bases, `ai-memory-management-for-llms` is 20.4 on 22 impressions versus 18.2 on 5 at 28 days; over 90 days it leads 22.7 on 48 versus 28.7 on 15. The named-human floor is 37.0 (n=1) versus 87.0 (n=1), with only 17.6% page+query coverage. Those denominators do not support a precise ranking claim, but they do invalidate the only premise for the reversal. **The original owner is restored: `ai-memory-management-for-llms`.** A 2.2-place short-window edge on five impressions cannot justify an irreversible redirect, and inbound links are retrofitted by the merge rather than treated as an independent winner signal.
