...
for path in sorted((ROOT / "content/posts").glob("*.md")):
    ...
    assert stripped == re.sub(rb"^work_note: .*\n", b"", old, flags=re.M), name
    note = post["work_note"]
    assert mapping[name]["note"] == note, name
    plain = re.sub(r"\[([^]]+)\]\([^)]+\)", r"\1", note)
    assert plain.count("Ninad Pathak") == 1, name
    assert not re.search(r"\b(?:I|me|my|mine|we|our|ours|us)\b|let['’]s", plain, re.I), name
    assert re.search(r"technical(?: \w+){0,2} (?:content|writer|documentation|comparisons|guides)|developer (?:content|tutorials)", plain, re.I), name
...
for name in ["build.py", "templates/base.html", "templates/post.html", "static/css/main.css"]:
