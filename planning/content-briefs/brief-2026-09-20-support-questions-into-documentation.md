...
Convert recurring community questions into pages that answer them permanently.

## Owns

The conversion loop: spotting a repeat question, deciding whether it is a docs gap, writing the
page, and closing the loop back to the community.
...
This is genuinely his work: turning recurring questions into documentation is what a technical
writer does. The honest content is the decision rule for *which* questions become pages, because
most should not.

Real ground he can point at: this site's own FAQ blocks are built from question-shaped queries, and
`build.py` generates FAQPage schema from `faqs` frontmatter.

**Do not claim a support-volume reduction, a client, or a ticket count.**
